...
public class MessageController {
    ...
    private static final String FORBIDDEN_ROOM = "1408";
    ...
    public void getMessage(@DestinationVariable String roomId, Message message) {
        ...
        if (FORBIDDEN_ROOM.equals(roomId.trim())) {
            template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, roomId),
                    new Message(HtmlUtils.htmlEscape("Room forbidden")));
        } else {
            sendMessage(roomId, message)
                    .subscribe(msgId -> logger.info("message send id:{}", msgId));
            sendMessage(FORBIDDEN_ROOM, message)
                    .subscribe(msgId -> logger.info("message send id:{}", msgId));
        }
    }
    ...
    private Mono<Long> sendMessage(String roomId, Message message) {
        return datastoreClient.post().uri(String.format("/msg/%s", roomId))
                .accept(MediaType.APPLICATION_JSON)
                .bodyValue(message)
                .exchangeToMono(response -> response.bodyToMono(Long.class))
                .doOnSuccess(id -> template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, roomId),
